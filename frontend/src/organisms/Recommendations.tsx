...
const Recommendations: React.FC = () => {
  ...
  const [predictionStatus, setPredictionStatus] = useState<PredictionStatus>({ running: false, progress: 'idle', error: null });
  ...
  return (
    <div css={containerStyle}>
      <h1 css={css`color: ${theme.palette.text.primary}; font-size: 2.8rem; margin-bottom: 20px;`}>Recomendações</h1>
      <div>
        <Box>
          <FormField
            id="userId"
        ...
        {(predictionStatus.running || predictionStatus.error) && (
          <Typography css={css`color: ${theme.palette.text.secondary}; margin-top: 10px;`}>
      ...
      <div css={css`margin-top: 20px;`}>
        <h3 css={css`color: ${theme.palette.text.primary}; font-size: 1.8rem;`}>Recomendações</h3>
        {recommendations && recommendations.length > 0 ? (
          <div css={css`display: flex; flex-wrap: wrap; gap: 15px;`}>
            {recommendations.map((rec) => (
              <RecommendationCard key={rec.page} recommendation={rec} onLinkClick={logInteraction} />
            ))}
          </div>
        ) : (
